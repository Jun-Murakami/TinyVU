# TinyVU Windows Release Build Script
...
# Start build process
Write-Header "TinyVU $Version Build Script"
...
cmake --build . --config $Configuration --target TinyVU_VST3
if ($LASTEXITCODE -ne 0) {
...
cmake --build . --config $Configuration --target TinyVU_Standalone
if ($LASTEXITCODE -ne 0) {
...
if ($BuildAAX) {
    ...
    cmake --build . --config $Configuration --target TinyVU_AAX
    if ($LASTEXITCODE -ne 0) {
...
$SourceVST3 = "$BuildDir\plugin\TinyVU_artefacts\$Configuration\VST3\TinyVU.vst3"
$DestVST3 = "$OutputDir\Windows\TinyVU.vst3"
...
$SourceStandalone = "$BuildDir\plugin\TinyVU_artefacts\$Configuration\Standalone\TinyVU.exe"
$DestStandalone = "$OutputDir\Windows\TinyVU.exe"
...
if ($BuildAAX) {
    ...
    $SourceAAX = "$BuildDir\plugin\TinyVU_artefacts\$Configuration\AAX\TinyVU.aaxplugin"
    $DestAAX = "$OutputDir\Windows\TinyVU.aaxplugin"
    
    if (Test-Path $SourceAAX) {
        # 既存の AAX バンドルがある場合は削除してからコピーする。
        # これを行わないと、Copy-Item が入れ子の
        # TinyVU.aaxplugin\TinyVU.aaxplugin\... を作ってしまう。
        ...
        $AAXBinary = Join-Path $DestAAX "Contents\x64\TinyVU.aaxplugin"
        ...
        if (Test-Path $WrapToolPath) {
            ...
            $PfxCandidates = @(
                ...
                "$RootDir\tinyvu-dev.pfx",         # Project root
                "$env:USERPROFILE\.tinyvu\dev.pfx", # User home directory
                ".\certificates\tinyvu-dev.pfx"    # Certificates folder
            )
            ...
            if (-not $PfxPath) {
                ...
                # WrapGUID（TinyVU 固有の PACE Organization GUID）は `.env` の PACE_ORGANIZATION に書く。
                # 姉妹リポジトリ（ZeroComp 等）の `.env` を雛形にして本プラグインの GUID に差し替えて使う。
                # スクリプト / ドキュメントには生 GUID をハードコードしない方針。
                $WrapGUIDToUse = $env:PACE_ORGANIZATION

                $SkipAAXSigning = $false
                $RequiredEnvVars = @("PACE_USERNAME", "PACE_PASSWORD", "PACE_KEYPASSWORD", "PACE_ORGANIZATION")
                $MissingVars = @()
...
$ReadmeContent = @"
TinyVU $Version - Windows Installation Guide
====================================================
...
3. For Standalone Application:
   Copy TinyVU.exe to any preferred location, for example:
   C:\Program Files\TinyVU\ or your Desktop.
...
if (Test-Path $InnoSetupPath) {
    ...
    if (Test-Path $InstallerScript) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $InstallerPath = "$OutputDir\TinyVU_${Version}_Windows_Setup.exe"
...
if ($BuildAAX) {
    $ZipName = "TinyVU_${Version}_Windows_VST3_AAX_Standalone.zip"
} else {
    $ZipName = "TinyVU_${Version}_Windows_VST3_Standalone.zip"
}
...
Write-Host "[✓] TinyVU.vst3 (with embedded WebUI)" -ForegroundColor Green
Write-Host "[✓] TinyVU.exe (Standalone application)" -ForegroundColor Green
if ($BuildAAX) {
    Write-Host "[✓] TinyVU.aaxplugin ($AAXSigningStatusSummary)" -ForegroundColor Green
}
...
if (Test-Path "$OutputDir\TinyVU_${Version}_Windows_Setup.exe") {
