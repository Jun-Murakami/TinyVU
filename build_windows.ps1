...
cmake --build . --config $Configuration --target TinyVU_CLAP
if ($LASTEXITCODE -ne 0) {
...
$SourceCLAP = "$BuildDir\plugin\TinyVU_artefacts\$Configuration\CLAP\TinyVU.clap"
$DestCLAP = "$OutputDir\Windows\TinyVU.clap"
...
4. For CLAP Plugin:
   Copy TinyVU.clap to the following location:
...
if ($BuildAAX) {
    $ZipName = "TinyVU_${Version}_Windows_VST3_AAX_CLAP_Standalone.zip"
} else {
    $ZipName = "TinyVU_${Version}_Windows_VST3_CLAP_Standalone.zip"
}
...
Write-Host "[✓] TinyVU.clap (CLAP plugin)" -ForegroundColor Green
if ($BuildAAX) {
