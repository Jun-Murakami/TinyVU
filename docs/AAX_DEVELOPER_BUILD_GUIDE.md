...
1. **依存ライブラリの確認**
   ...
   depends.exe TinyVU.aaxplugin

   # macOS
   otool -L TinyVU.aaxplugin/Contents/MacOS/TinyVU
   ```
...
2. **ログ機能の実装**
   ...
   // AAXプラグイン内でのログ
   #ifdef DEBUG
   AAX_TRACE(AAX_TRACE_PRIORITY_NORMAL, "TinyVU: %s", message);
   #endif
   ```
