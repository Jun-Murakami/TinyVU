...
# TinyVU macOS Release Build Script (zsh)
...
echo_header "TinyVU ${VERSION} Build Script (macOS zsh)"
...
EXISTING_ARTIFACTS_DIR="${BUILD_DIR}/plugin/TinyVU_artefacts/${CONFIGURATION}"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target TinyVU_VST3 TinyVU_AU TinyVU_Standalone TinyVU_CLAP TinyVU_AAX
else
    cmake --build "${BUILD_DIR}" --config "${CONFIGURATION}" --target TinyVU_VST3 TinyVU_AU TinyVU_Standalone TinyVU_CLAP
fi
echo_success "Plugin build completed"

# Artifact paths
ARTIFACTS_DIR="${BUILD_DIR}/plugin/TinyVU_artefacts/${CONFIGURATION}"

# Verify Universal Binary
echo_step "Checking architecture of built plugins..."
if [[ -f "${ARTIFACTS_DIR}/VST3/TinyVU.vst3/Contents/MacOS/TinyVU" ]]; then
    echo "  VST3:"
    lipo -info "${ARTIFACTS_DIR}/VST3/TinyVU.vst3/Contents/MacOS/TinyVU"
fi
if [[ -f "${ARTIFACTS_DIR}/AU/TinyVU.component/Contents/MacOS/TinyVU" ]]; then
    echo "  AU:"
    lipo -info "${ARTIFACTS_DIR}/AU/TinyVU.component/Contents/MacOS/TinyVU"
fi
if [[ -f "${ARTIFACTS_DIR}/Standalone/TinyVU.app/Contents/MacOS/TinyVU" ]]; then
    echo "  Standalone:"
    lipo -info "${ARTIFACTS_DIR}/Standalone/TinyVU.app/Contents/MacOS/TinyVU"
fi
if [[ -f "${ARTIFACTS_DIR}/CLAP/TinyVU.clap/Contents/MacOS/TinyVU" ]]; then
    echo "  CLAP:"
    lipo -info "${ARTIFACTS_DIR}/CLAP/TinyVU.clap/Contents/MacOS/TinyVU"
fi
if [[ ${BUILD_AAX} -eq 1 ]] && [[ -f "${ARTIFACTS_DIR}/AAX/TinyVU.aaxplugin/Contents/MacOS/TinyVU" ]]; then
    echo "  AAX:"
    lipo -info "${ARTIFACTS_DIR}/AAX/TinyVU.aaxplugin/Contents/MacOS/TinyVU"
fi
echo_success "Architecture verification completed"
SRC_VST3="${ARTIFACTS_DIR}/VST3/TinyVU.vst3"
SRC_AU="${ARTIFACTS_DIR}/AU/TinyVU.component"
SRC_APP="${ARTIFACTS_DIR}/Standalone/TinyVU.app"
SRC_CLAP="${ARTIFACTS_DIR}/CLAP/TinyVU.clap"
...
# AAX check
if [[ ${BUILD_AAX} -eq 1 ]]; then
    SRC_AAX="${ARTIFACTS_DIR}/AAX/TinyVU.aaxplugin"
    if [[ ! -d "${SRC_AAX}" ]]; then echo_error "AAX not found: ${SRC_AAX}"; exit 1; fi
fi

echo_step "Collecting artifacts..."
DEST_VST3="${OUTPUT_DIR}/TinyVU.vst3"
DEST_AU="${OUTPUT_DIR}/TinyVU.component"
DEST_APP="${OUTPUT_DIR}/TinyVU.app"
DEST_CLAP="${OUTPUT_DIR}/TinyVU.clap"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    DEST_AAX="${OUTPUT_DIR}/TinyVU.aaxplugin"
    rm -rf "${DEST_AAX}"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ...
    #   PACE_KEYPASSWORD  : PFX パスワード
    #   PACE_ORGANIZATION : 当該プラグイン固有の WrapGUID（旧 PACE_WCGUID も互換のため受ける）
    # 生 GUID はスクリプト / ドキュメントにハードコードしない。`.env` に書く運用に統一。
...
if [[ -f "${DEST_VST3}/Contents/MacOS/TinyVU" ]]; then
    INFO="$(lipo -info "${DEST_VST3}/Contents/MacOS/TinyVU" 2>/dev/null || true)"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
   ...
   Copy the entire TinyVU.aaxplugin folder to the following location:
...
TinyVU ${VERSION} - macOS Installation Guide
====================================================
...
2. For VST3 Plugin:
   Copy the entire TinyVU.vst3 folder to the following location:
   ~/Library/Audio/Plug-Ins/VST3/

3. For Audio Unit (AU):
   Copy the entire TinyVU.component folder to the following location:
   ~/Library/Audio/Plug-Ins/Components/

4. For Standalone Application:
   Copy TinyVU.app to any preferred location, for example:
   /Applications/ or your Desktop.

5. For CLAP Plugin:
   Copy the entire TinyVU.clap folder to the following location:
...
PKG_ID_BASE="${PKG_ID_BASE:-com.bucketrelay.tinyvu}"
...
PKG_VST3="${PKG_WORK_DIR}/TinyVU_VST3.pkg"
pkgbuild \
    --root "${PKGROOT_VST3}" \
...
LOGIC_CATEGORY="${LOGIC_CATEGORY:-Metering}"
...
PKG_AU="${PKG_WORK_DIR}/TinyVU_AU.pkg"
pkgbuild \
    --root "${PKGROOT_AU}" \
...
PKG_APP="${PKG_WORK_DIR}/TinyVU_Standalone.pkg"
pkgbuild \
    --component "${DEST_APP}" \
...
PKG_CLAP="${PKG_WORK_DIR}/TinyVU_CLAP.pkg"
pkgbuild \
    --root "${PKGROOT_CLAP}" \
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ...
    PKG_AAX="${PKG_WORK_DIR}/TinyVU_AAX.pkg"
    pkgbuild \
        --root "${PKGROOT_AAX}" \
...
fi
{
    ...
    echo "  <title>TinyVU ${VERSION}</title>"
    ...
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.vst3\">TinyVU_VST3.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.au\">TinyVU_AU.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.app\">TinyVU_Standalone.pkg</pkg-ref>"
    echo "  <pkg-ref id=\"${PKG_ID_BASE}.clap\">TinyVU_CLAP.pkg</pkg-ref>"
    if [[ ${BUILD_AAX} -eq 1 ]]; then
        echo "  <pkg-ref id=\"${PKG_ID_BASE}.aax\">TinyVU_AAX.pkg</pkg-ref>"
    fi
...
PRODUCT_PKG_PATH="${OUTPUT_DIR}/../TinyVU_${VERSION}_macOS.pkg"
...
if [[ ${BUILD_AAX} -eq 1 ]]; then
    ZIP_NAME="TinyVU_${VERSION}_macOS_VST3_AU_AAX_CLAP_Standalone.zip"
else
    ZIP_NAME="TinyVU_${VERSION}_macOS_VST3_AU_CLAP_Standalone.zip"
fi
