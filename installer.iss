...
[Setup]
AppId={{52EC344B-36B5-432A-B6B9-224DA0E0F7AC}
AppName={#MyAppName}
...
OutputBaseFilename=TinyVU_{#MyAppVersion}_Windows_Setup
...
Source: "build\plugin\TinyVU_artefacts\Release\Standalone\TinyVU.exe"; DestDir: "{code:GetStandalonePath}"; Flags: ignoreversion; Components: standalone
Source: "build\plugin\TinyVU_artefacts\Release\Standalone\*.dll"; DestDir: "{code:GetStandalonePath}"; Flags: ignoreversion skipifsourcedoesntexist; Components: standalone

; VST3 Plugin
Source: "build\plugin\TinyVU_artefacts\Release\VST3\TinyVU.vst3\*"; DestDir: "{code:GetVST3Path}\TinyVU.vst3"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: vst3

; CLAP Plugin — 単一ファイル（.clap = DLL に拡張子を付けたもの）
Source: "releases\{#MyAppVersion}\Windows\TinyVU.clap"; DestDir: "{code:GetCLAPPath}"; Flags: ignoreversion; Components: clap

; AAX Plugin — バンドル全体を丸ごとコピー（入れ子は PrepareToInstall で事前削除により防止）
Source: "releases\{#MyAppVersion}\Windows\TinyVU.aaxplugin\*"; DestDir: "{code:GetAAXPath}\TinyVU.aaxplugin"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: aax
...
Type: filesandordirs; Name: "{localappdata}\TinyVU"
Type: filesandordirs; Name: "{userappdata}\TinyVU"

[Registry]
; Register VST3 plugin location
Root: HKLM64; Subkey: "Software\VST3"; ValueType: string; ValueName: "TinyVU"; ValueData: "{code:GetVST3Path}\TinyVU.vst3"; Flags: uninsdeletevalue; Components: vst3

; Register CLAP plugin location（CLAP 規約に厳密なレジストリ要件は無いが、参照用に同じパターンで残す）
Root: HKLM64; Subkey: "Software\CLAP"; ValueType: string; ValueName: "TinyVU"; ValueData: "{code:GetCLAPPath}\TinyVU.clap"; Flags: uninsdeletevalue; Components: clap

; Register AAX plugin location
Root: HKLM64; Subkey: "Software\Avid\ProTools\AAX"; ValueType: string; ValueName: "TinyVU"; ValueData: "{code:GetAAXPath}\TinyVU.aaxplugin"; Flags: uninsdeletevalue; Components: aax
...
begin
  ...
  PathSelectionPage.Values[0] := ExpandConstant('{autopf}\TinyVU');
  ...
  begin
    ...
    begin
      if MsgBox('Microsoft Visual C++ Redistributables are required but not installed.' + #13#10 + #13#10 +
                ...
                '(Required for TinyVU to run properly)', mbConfirmation, MB_YESNO) = IDYES then
      begin
        DownloadPage.Clear;
        ...
        try
          ...
          MsgBox('Failed to download Visual C++ Redistributables.' + #13#10 +
                 'TinyVU may not work properly without them.' + #13#10 + #13#10 +
      ...
      begin
        MsgBox('Warning: TinyVU may not work without Visual C++ Redistributables.' + #13#10 + #13#10 +
...
begin
  ...
  begin
    ...
    TestFile := TargetPath + '\TinyVU.vst3\Contents\x86_64-win\TinyVU.vst3';
    ...
    begin
      ...
      begin
        ...
        if MsgBox('The VST3 plugin appears to be in use by a DAW.' + #13#10 + #13#10 +
                  'Please close any DAW that might be using TinyVU and try again.' + #13#10 + #13#10 +
  ...
  begin
    ...
    TestFile := TargetPath + '\TinyVU.clap';
    if FileExists(TestFile) then
    begin
      FileLocked := not DeleteFile(TestFile);
      if FileLocked then
  ...
  begin
    ...
    if DirExists(TargetPath + '\TinyVU.aaxplugin') then
    begin
      Log('Removing existing AAX bundle to avoid nested installation');
      if not DelTree(TargetPath + '\TinyVU.aaxplugin', True, True, True) then
      begin
  ...
  begin
    ...
    if StandalonePath <> '' then
      ...
      TargetPath := ExpandConstant('{autopf}\TinyVU');

    TestFile := TargetPath + '\TinyVU.exe';
    if FileExists(TestFile) then
    begin
      FileLocked := not DeleteFile(TestFile);
      if FileLocked then
      begin
        ...
        if MsgBox('TinyVU standalone application appears to be running.' + #13#10 + #13#10 +
                  'Please close it and try again.' + #13#10 + #13#10 +
                  'Continue anyway? (Installation may fail)', mbConfirmation, MB_YESNO) = IDNO then
        begin
          Result := 'TinyVU.exe is running. Please close it and try again.';
          Exit;
        end;
      end;
    end;
...
begin
  if StandalonePath = '' then
    Result := ExpandConstant('{autopf}\TinyVU')
  else
...
begin
  ...
  DelTree(ExpandConstant('{localappdata}\TinyVU'), True, True, True);
  DelTree(ExpandConstant('{userappdata}\TinyVU'), True, True, True);
end;
