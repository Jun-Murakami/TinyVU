...
/**
 * public/ (共有) と public-web/ (Web 専用 = サンプル mp3 等) の両方を配信する Vite プラグイン。
 *  - Dev: public-web/ を追加の静的ファイルディレクトリとして配信
 *  - Build: public-web/ の内容を dist/ にコピーし、index.web.html を index.html にリネーム
 */
...
/**
 * Web (SPA) ビルド用 Vite 設定（TinyVU）。
 *
 * 主な違い (vite.config.ts と比較):
 * 1. juce-framework-frontend-mirror → bridge/web/juce-shim.ts にエイリアス
 * 2. bridge/juce → bridge/web/web-juce.ts にエイリアス
 * 3. 出力先: dist/（プラグインビルドは ../plugin/ui/public）
 * 4. VITE_RUNTIME=web 環境変数（App.tsx で IS_WEB_MODE 分岐に使う）
 * 5. public-web/ から sample.mp3 等を配信
 */
